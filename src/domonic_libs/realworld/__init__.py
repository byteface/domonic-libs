...
# a bundle that resolved to its CommonJS build rather than a browser one --
# retry these once with `commonjs=True` (myjs's `require` / `module` / `exports`)
_CJS_ERR = re.compile(r"\b(?:exports|module|require)\b is not defined")


def _cjs_export_check(interp: Interpreter) -> Case:
    """The CommonJS path exports through `module.exports`, not a global."""
    mod = interp.global_env.get("module")
    exp = mod.get("exports") if isinstance(mod, dict) else None
    if isinstance(exp, dict) and any(not k.startswith("_") for k in exp):
        names = sorted(k for k in exp if not k.startswith("_"))
        return Case("exposes a real global", "PASS",
                    "module.exports: " + ", ".join(names[:6])
                    + (f" (+{len(names) - 6} more)" if len(names) > 6 else ""))
    if exp not in (None, {}) and callable(exp):
        return Case("exposes a real global", "PASS", "module.exports is a function")
    return Case("exposes a real global", "FAIL", "loaded, but module.exports stayed empty")


def _eval_library(name: str, library_src: str, *, commonjs: bool) -> FileResult:
    g, console, document, window = default_globals()
    interp = Interpreter(g, global_object=window, commonjs=commonjs)
    try:
    ...
    check = _cjs_export_check(interp) if commonjs else _generic_export_check(after - before)
    return FileResult(name, [check])
...
def _run_library(name: str, url: str) -> FileResult:
    ...
    result = _eval_library(name, library_src, commonjs=False)
    # jsdelivr's bare `npm/<pkg>` often resolves to a package's CommonJS
    # build. That's not what a `<script src>` loads, but myjs *does* run
    # CommonJS -- retry so the sweep exercises the module path too.
    if result.error and _CJS_ERR.search(result.error):
        retry = _eval_library(name, library_src, commonjs=True)
        if not retry.error:
            return retry
        low = retry.error.lower()
        if "find module" in low or "no module named" in low:
            # a multi-file CommonJS package -- the real reason it won't load
            # as one bundle, more informative than "exports is not defined"
            return FileResult(name, [], error=retry.error + " [multi-file CommonJS package]")
    return result
...
def format_markdown(result: SuiteResult) -> str:
    ...
    for f in result.files:
        ...
        note = note.replace(f" from {cwd}", "").replace(cwd + "/", "")   # keep local paths out of the doc
